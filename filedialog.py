...
def getfile(initpath, multi=False):
    ...
    if multi:
        Tk().withdraw()
        filename = dlg.askopenfilenames(initialdir=initpath, title="Select file", filetypes=types)
    else:
        Tk().withdraw()
        filename = dlg.askopenfilename(initialdir=initpath, title="Select files", filetypes=types)
